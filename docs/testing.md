...
Ubuntu apt 的 Lazarus 3.0 只有 GTK2/Qt5，没有 LCL GTK3 包。自行安装 FPC + Lazarus（GTK3），用 `FPC` / `LAZARUS_DIR` 指向它们。音频库走发行版 FFmpeg/SDL2（pkg-config）。包列表与环境变量见 README 构建一节。

```bash
# FFmpeg / SDL2 / GTK3 / X11 开发包（libttcore.so + LCL GTK3）
